    ...
    it('async and toUrl works', function (done) {
        ...
        modulex.use(['t/t1', 't/t2'], function (t1, t2) {
            ...
            Q.all([ret1.promise, ret2.promise]).then(function (v) {
                var ee;
                try {
                    expect(v[0].css).to.be.equal('http://' + location.host + '/tests/specs/require-api/x.css?t=2.css');
                    expect(v[0].swf).to.be.equal('http://' + location.host + '/tests/specs/require-api/x.swf?t=2.swf');
